...
describe("Chest API Client", () => {
  ...
  it("delete should throw on error", async () => {
    ...
    await expect(chestAPI.delete(999)).rejects.toThrow(
      "Failed to delete chest",
    );
  });
});
...
describe("Source API Client", () => {
  ...
  it("getByChest should fetch sources for a chest", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      `${API_BASE}/api/sources/?chest_id=1`,
    );
  });
  ...
  it("create should POST and return new source", async () => {
    ...
    const result = await sourceAPI.create({
      chest_id: 1,
      name: "New",
      type: "TXT",
      content: "text",
...
describe("Chat API Client", () => {
  ...
  it("query should throw on error", async () => {
    mockFetch.mockResolvedValue(new Response(null, { status: 500 }));
    await expect(
      chatAPI.query({ question: "test?", chest_id: 1 }),
    ).rejects.toThrow();
  });

  it("streamQuery should call onChunk and onDone", async () => {
    const sseData =
      "data: Hello[ENDLINE]data: World[ENDLINE]data: [DONE][ENDLINE]";
